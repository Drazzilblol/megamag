...
public class EventDetailsActivity extends MvpAppCompatActivity implements EventDetailsView, OnPaletteReadyCallback {
    ...
    @BindView(R.id.activity_details_tab_layout)
    TabLayout tabLayout;
    ...
    public void showData(@NonNull Event event) {
        ImageUtils.loadImageWithBlur(this, coverView, event.getCoverUrl(), this);
    ...
    public void onPaletteReady(Palette palette) {
        ...
        if (swatch != null) {
            int commonColor = swatch.getRgb();
            int textColor = swatch.getBodyTextColor();
            collapsingToolbarLayout.setExpandedTitleColor(ImageUtils.adjustAlpha(textColor, 230));
            collapsingToolbarLayout.setCollapsedTitleTextColor(ImageUtils.adjustAlpha(textColor, 230));
            collapsingToolbarLayout.setContentScrimColor(commonColor);
